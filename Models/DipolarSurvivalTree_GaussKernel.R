...
DipolarSurvivalTree_GaussKernel = R6Class(
  
  
  
  classname = "DipolarSurvivalTree_GaussKernel",
  ...
  public = list(
    ...
    K.bi = function(X) {
      XminXsqr = apply(X, MARGIN = 1, FUN = function(xi) {
        apply(X, MARGIN = 1, FUN = function(xj) {
          sum((xi - xj)^2)
        })
      })
      exp(-XminXsqr/(2*self$Ksigma))
    },
    ...
    ridgedipolarcriterion = function(betapm, K.XX, mupmdiff, w0) {
      ...
      Psi_betapm_w_w0_kernalized = sum(betapm * phipm_kernelized)
